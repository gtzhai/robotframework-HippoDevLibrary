...
_eel_js_file = pkg.resource_filename('HippoDevLibrary', 'eel.js')
...
_start_args = {
    ...
    'port':             8123,                       # Port used for Bottle server (use 0 for auto)
...
def _process_message(message, ws):
    if 'call' in message:
        return_val = _exposed_functions[message['name']](hippo_dev, *message['args'])
